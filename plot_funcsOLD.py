...
def getContrastBehData(recordingList):
    """
    Extract per-session contrast-response probabilities from behavioural CSVs.

    Returns
    -------
    dict with keys:
        'session_data'   : list of dicts, one per session {'name', 'contrasts', 'probs'}
        'contrast_probs' : dict, contrast (float) -> list of per-session probabilities
    """
    EXCLUDE = np.array([-0.0625, 0.0625], dtype=float)
    session_data   = []
    ...
    for ind in range(len(recordingList)):
        ...
        try:
            ...
            if contrasts:
                session_data.append({'name': session, 'contrasts': contrasts, 'probs': probs})
    ...
    for ind, recordingDate in enumerate(recordingList.recordingDate):
        if recordingList.imagingDataExtracted[ind] == 1:
            ...
            try:
                ...
                session_contrasts = []
                # For this session, save the points temporarily
                session_contrasts = []
                session_means = []
                session_means_by_contrast = {}

                # Calculate mean df/f for each rewarded contrast
                for i, contrast in enumerate(contrasts_rewarded):
                    if contrast in dffTrace_mean and dffTrace_mean[contrast] is not None:
                        if dffTrace_mean[contrast].shape[1] > end_frame:
                            ...
                            baseline = np.nanmean(dffTrace_mean[contrast][:, baseline_start_idx:baseline_end_idx], axis=1, keepdims=True)
                            ...
                            session_means_by_contrast[contrast_values[i]] = mean_dff
                            session_contrasts.append(contrast_values[i])
                            session_means.append(mean_dff)

                # ---- zero-align within session ----
                if 0.0 in session_means_by_contrast:
                    zero_val = session_means_by_contrast[0.0]

                    aligned_contrasts = []
                    aligned_means = []

                    for c in session_contrasts:
                        aligned_mean = session_means_by_contrast[c] - zero_val

                        contrast_data[contrasts_rewarded[contrast_values.index(c)]].append({
                            'animal': animal_id,
                            'session': session_name,
                            'mean_dff': aligned_mean,
                            'contrast': c
                        })

                        aligned_contrasts.append(c)
                        aligned_means.append(aligned_mean)

                    if len(aligned_contrasts) > 1:
                        session_points[session_name] = (aligned_contrasts, aligned_means)  
...
def plotMeanDffByContrast(
    ...
    - optional within-session normalization
    - optional hemisphere alignment:
        negative x = ipsi
        positive x = contra

    Parameters
    ----------
    align_to_hemisphere : bool
        If True, flips contrast sign for right hemisphere recordings so that:
            left side of x-axis  = ipsi
            right side of x-axis = contra

    session_normalization : str or None
        None                -> raw baseline-subtracted values
        'zero_center'       -> subtract session response at 0 contrast
        'zero_center_scale' -> subtract 0 contrast response, then divide by max abs response
    """
...
def createMeanDffByContrastData(
    ...
    """
    Create session-level and pooled mean dF/F by contrast data.

    Returns
    -------
    result : dict with keys
        'contrast_data'   : dict, keyed by contrast, each value is a list of dicts
        'session_points'  : dict, keyed by session_name -> (xvals, yvals)
        'summary_df'      : pd.DataFrame with one row per session x contrast
        'mean_df'         : pd.DataFrame with pooled mean/sem per contrast
        'meta'            : dict of settings
    """
    ...
    # ---- fixed frame window (computed once if not per-trial) ----
    per_trial = time_window is None
    if not per_trial:
        sf_fixed = int((time_window[0] + pre_stim_sec) * fRate_imaging)
        ef_fixed = int((time_window[1] + pre_stim_sec) * fRate_imaging)

    pkl_name = 'imaging-dffTrace_zscored.pkl' if use_zscored else 'imaging-dffTrace.pkl'
    ...
    def _metric(trace, sf, ef):
        with np.errstate(all='ignore'):
            ...
            if per_trial:
                rm = np.stack(
                    [np.nanmean(dff[:, sf[t]:ef[t], t], axis=1) for t in range(dff.shape[2])],
                    axis=1
                )
            else:
                rm = np.nanmean(dff[:, sf:ef, :], axis=1)  # (n_cells, n_trials) — vectorized
            return np.nanmean(rm, axis=1)                   # (n_cells,)
    ...
    for ind in range(len(recordingList.recordingDate)):
        ...
        sf, ef = (
            mfun.get_startFramePerTrial(recordingList.analysispathname[ind],
                                        recordingList.sessionName[ind], event_type)
            if per_trial else (sf_fixed, ef_fixed)
        )
        ...
        with open(os.path.join(subfolder_path, pkl_name), 'rb') as f:
            dffTrace_mean = pickle.load(f)[pkl_key]

        # ---- available contrasts for this session ----
        available = [
            (i, c) for i, c in enumerate(contrasts_rewarded)
            if c in dffTrace_mean and dffTrace_mean[c] is not None
            if c in dffTrace_mean and isinstance(dffTrace_mean[c], np.ndarray)
        ]
        if not available:
            print(f"Skipping {session_name}: no valid contrast arrays in pkl")
            continue

        # ---- cellIndex from highest contrast (computed once per session) ----
        ref_metric = _metric(dffTrace_mean[available[-1][1]], sf, ef)
        if response_type == 'excited':
        ...
        for i, contrast in available:
            ...
            with np.errstate(all='ignore'):
                cell_max = np.nanmax(np.abs(cell_responses))
            if cell_max > 0 and not np.isnan(cell_max):
                cell_responses = cell_responses / cell_max
...
def contrastHemisphereDifference (data_out):
    ...
    for c in contrast_levels:
        ...
        if c == 0:
            ...
            zero_df['contrast'] = 0
            zero_df['left_minus_right'] = zero_df['mean_dff']
            rows.append(zero_df)

        else:
            left = df[df['contrast'] == -c][['session', 'mean_dff']]
            right = df[df['contrast'] == c][['session', 'mean_dff']]

            merged = pd.merge(left, right, on='session', suffixes=('_left', '_right'))

            merged['contrast'] = c
            merged['left_minus_right'] = merged['mean_dff_left'] - merged['mean_dff_right']
    ...
    contrast_summary = (
        contrast_diff_df
        .groupby('contrast')['left_minus_right']
        .agg(['mean','sem','count']))
    return contrast_summary

def create_glm_dataframe(data_out, recordingList, bias_col):
    """
    ...
    meta_cols = ['sessionName', 'recordingHemisphere', bias_col]
